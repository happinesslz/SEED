...
class MultiScaleBEVResBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        self.block_convs.append(nn.Conv2d(128, 128, 1, stride=1, bias=False))
        self.block_convs.append(nn.Conv2d(128, 128, 1, stride=1, bias=False))
        self.block_convs.append(nn.Conv2d(128, 128, 1, stride=1, bias=False))
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(self.blocks)):
            ...
            backbone_x_conv_i = data_dict['multi_scale_2d_features']['x_conv%d' % (i + 3)].dense()
            x = self.block_convs[i](backbone_x_conv_i) + x

            stride = int(spatial_features.shape[2] / x.shape[2])
            ret_dict['spatial_features_%dx' % stride] = x  ## save multi-scale features
